...
- The first step with the reconciliation of the free text fields is to first look for identical labels and again select the most common. If no identical labels are found we first use a normalization step where we remove extra white spaces and again look for matches (e.g. ‘M.   Denslow’ will be normalized matched to ‘M. Denslow’). 
...
### Summary of Free - Text Reconciliation process:
    ...
    normalized exact match = removed white spaces and checks for a match 
