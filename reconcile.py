...
def parse_command_line():
    ...
    parser = argparse.ArgumentParser(description='''
        This takes raw Notes from Nature classifications and creates a reconciliation of the
        classifications for a particular workflow. That is, it reduces n classifications per subject
        to the "best" values. The summary file will provide explanations of how the reconciliations
        were done.
    ''')
    parser.add_argument('classifications',
                        help='The Notes from Nature classifications CSV input file.')
    parser.add_argument('-w', '--workflow-id', type=int,
                        help='The workflow to extract. Required if there is more than one workflow '
                             'in the classifications file.')
...
if __name__ == "__main__":
    ...
    UNRECONCILED_DF = create_unreconciled_dataframe(ARGS.workflow_id, ARGS.classifications)
