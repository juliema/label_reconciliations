...
def extract_annotations_json(unreconciled_df):
    """Extract annotations from a json object. Annotations are a deeply nested
    json blob with a peculiar data format. We use this function to gather the
    data type so that we can hoist the data later on.
    """

    unreconciled_df['annotation_json'] = unreconciled_df['annotations'].map(
        json.loads)
...
def extract_subject_json(unreconciled_df):
    ...
    for k in subject_keys:
        unreconciled_df['subject_' + k] = \
            unreconciled_df['subject_json'].apply(extract_json_value, column=k)
...
def create_unreconciled_dataframe(workflow_id, classifications):
    ...
    if 'subject_id' in unreconciled_df.columns:
        unreconciled_df.rename(columns={'subject_id': 'subject_id_external'},
                               inplace=True)
    ...
    unreconciled_df.sort_values(['subject_id', 'classification_id'],
                                inplace=True)
