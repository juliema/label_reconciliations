...
def format_explanation(form, record_count=None, blank_count=None,
    ...
    total_words = TOTAL_PLURALS.copy()
    if record_count == 1:
        total_words = TOTAL_SINGULARS.copy()

    blank_words = BLANK_PLURALS.copy()
    if blank_count == 1:
        blank_words = BLANK_SINGULARS.copy()
...
def explain_no_match(values, filled, match_type):
    ...
    return (format_explanation(form,
                               record_count=record_count,
                               blank_count=blank_count,
                               match_type=match_type), '')
...
def best_select_value(group):
    ...
    values = [str(g) if str(g).lower() not in PLACE_HOLDERS else ''
              for g in group]
...
def best_text_value(group):
    ...
    if filled[0].count > 1:
        return explain_exact_match(filled[0].value,
                                   values,
                                   filled,
                                   'Normalized exact')
    ...
    if top.score >= ARGS.fuzzy_ratio_threshold:
        return explain_fuzzy_match(top.value,
                                   values,
                                   filled,
                                   top.score,
                                   'Partial ratio')
...
def create_reconciled_dataframes(unreconciled_df, args):
    ...
    global ARGS  # pandas' function signatures are not controlled by us
    # pylint: enable=global-statement
    ARGS = args

    # Aggregate columns based on each column's type keyed by column name
    method_for_select_columns = {col: best_select_value
                                 for col in unreconciled_df.columns
                                 if re.match(util.SELECT_COLUMN_PATTERN, col)}
    method_for_text_columns = {col: best_text_value
                               for col in unreconciled_df.columns
                               if re.match(util.TEXT_COLUMN_PATTERN, col)}
    method_for_subject_columns = {col: all_are_identical
                                  for col in unreconciled_df.columns
    ...
    reconciled_df = unreconciled_df.fillna('').groupby(util.GROUP_BY)
    ...
    explanations_df = reconciled_df.loc[:, [col
